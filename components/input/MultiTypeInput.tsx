...
export const MultiTypeInput = ({
	...
	const [activeInput, setActiveInput] = useState<string>(startingInput);
	...
	const stateUpdater =
		<T,>(
			kind: ItemDataKind,
			cacher: (val: T) => Promise<string>,
			encoder: (val: T) => Promise<Uint8Array>
		): ((val: T) => Promise<void>) =>
		async (val: T) => {
			...
			onChange(
				Object.values(newData)
					.filter((d) => d != undefined && d.kind && d.data)
					.map((d) => d as IdeaData)
			);
		};
	...
	const clearInputs = (kind: string) => {
		setData((data) => {
			return { ...data, [labelMeanings[kind]]: undefined };
		});
		setInputValues((inputValues) => {
			return { ...inputValues, [labelMeanings[kind]]: undefined };
		});
		setActiveInput("");
	};
	...
	return (
		<div>
			...
			<div
				className={`${styles.inputItems} ${headless ? styles.headless : ""}`}
			>
				<OutlinedOptionSelector
					options={Object.keys(labelMeanings)}
					activeOptions={
						new Set(
							Object.keys(labelMeanings).filter(
								(inputType) =>
									labelMeanings[inputType] in data &&
									data[labelMeanings[inputType]] !== undefined
							)
						)
					}
					onClear={clearInputs}
